									  ...
									  const bool &is_color_image)
{
	...
	{
		//std::cout << "sample_sz.width: " << img_sample_sz[0].width << 
		//	" input_sz: " << img_input_sz[0].width << 
		//	" scales: " << scales[0] << std::endl;
	...
	if (params.useHogFeature)
	{
		...
		bool useFast_HoG = false;
		if (useFast_HoG) {
			Fast_HoGFeature* FeatureVector = new Fast_HoGFeature();
			hog_feat_maps_ = FeatureVector->computeHOGFeature(img_samples[hog_feat_ind_]);
...
#endif	
		//std::cout << "vec36 is: " << hog_feat_maps_[0].at<float>(0, 0) << std::endl;
		//hog_feat_maps_ = hog_feature_normalization(hog_feat_maps_);
									   ...
									   cv::Size2f input_sz)
{
	...
	// Downsample factor
	// resize_factor is actually the currentScaleFactor in initialization
...
vector<cv::Mat> FeatureExtractor::hog_feature_normalization(vector<cv::Mat> &hog_feat_maps)
{
	...
	for (size_t i = 0; i < hog_feat_maps.size(); i++)
	{
		...
		else
		{
			...
			//std::cout << "sqrt(para / sum) is: " << sqrt(para / sum) << std::endl;
...
{
	vfc::int32_t f_startX = 1;
	vfc::int32_t dx, dy;
	for (vfc::int32_t y = f_startY; y < f_endY; y++) {
		for (vfc::int32_t x = f_startX; x < f_endX; x++) {
			if (f_inputImg.type() == CV_8U) {
				vfc::int32_t dx = vfc::int32_t(f_inputImg.at<uchar>(y, x + 1) - f_inputImg.at<uchar>(y, x - 1));
				vfc::int32_t dy = vfc::int32_t(f_inputImg.at<uchar>(y + 1, x) - f_inputImg.at<uchar>(y - 1, x));
			}
			else if (f_inputImg.type() == CV_32F) {
				vfc::int32_t dx = vfc::int32_t(f_inputImg.at<float>(y, x + 1) - f_inputImg.at<float>(y, x - 1));
				vfc::int32_t dy = vfc::int32_t(f_inputImg.at<float>(y + 1, x) - f_inputImg.at<float>(y - 1, x));
			}
			else {
				std::cout << "unknow input image type" << std::endl;
				return;
			}
			vfc::float64_t magnitude = ::hypot(dx, dy);
			bool l_interpolateBin = true;
			if (l_interpolateBin) {
				vfc::float64_t angle = atan2(static_cast<vfc::float64_t>(dy), static_cast<vfc::float64_t>(dx));
				...
				vfc::int32_t bin = vfc::int32_t(angle);
				vfc::float64_t interpol = angle - bin;  // fractional part of bin-assignment

				vfc::float64_t highbin = interpol*magnitude;  // voting value for high bin
				vfc::float64_t lowbin = magnitude - highbin;   // remaining voting value for the low bin

				f_integralImg(x + 1, y + 1).clear();
				f_integralImg(x + 1, y + 1).cnt[bin % 9] = vfc::int32_t(lowbin);
				f_integralImg(x + 1, y + 1).cnt[(bin + 1) % 9] = vfc::int32_t(highbin);
			}
			else
			{
				vfc::int32_t bin = getBinIdx(dx, dy);
				f_integralImg(x + 1, y + 1).clear();
				f_integralImg(x + 1, y + 1).cnt[bin % 9] = vfc::int32_t(magnitude);
			}
		}
	}
}
...
vfc::int32_t getBinIdx(vfc::uint8_t f_dx, vfc::uint8_t f_dy) {
	...
	if (f_dx == 0) {
		...
		vfc::float32_t l_dyBydx = static_cast<vfc::float64_t>(f_dy) / static_cast<vfc::float64_t>(f_dx);
		if (l_dyBydx >= 0.0f)// <90 degree
		{
			if (l_dyBydx<0.8391)  // <40
			{
				if (l_dyBydx<0.3640) // <20
				{
		...
		{
			l_dyBydx = -l_dyBydx;
			if (l_dyBydx <= 0.8391)  // [140, 180)
			{
				if (l_dyBydx <= 0.3640) // >=160
				{
					//160
					bin2 = 8;
				}
...
void integrateGradientX(vfc::int32_t f_startY, vfc::int32_t f_endY,
	...
	for (vfc::int32_t y = f_startY; y<f_endY; y++)
		for (vfc::int32_t x = f_startX; x<f_endX; x++)
		{
			f_integralImg(x, y).add(f_integralImg(x - 1, y));
		}
}
...
void integrateGradientY(vfc::int32_t f_startX, vfc::int32_t f_endX,
	...
	for (vfc::int32_t x = f_startX; x<f_endX; x++)
		for (vfc::int32_t y = f_startY; y<f_endY; y++)
		{
			f_integralImg(x, y).add(f_integralImg(x, y - 1));
		}
}

void computeIntegralHistogram(const cv::Mat& img, vfc::TImage<HoGHistogram9>& integral) {
	integral.resize(img.cols + 1, img.rows + 1);
	vfc::TImageView<HoGHistogram9> intg = integral.getView();
	vfc::int32_t w = img.cols, h = img.rows;
	// initialize top/left border with 0 (because we cannot compute a gradient there)
	for (vfc::int32_t y = 0; y <= h; y++) { intg(0, y).clear(); }
	for (vfc::int32_t x = 1; x <= w; x++) { intg(x, 0).clear(); }
	for (vfc::int32_t y = 0; y <= h; y++) { intg(1, y).clear(); }
	for (vfc::int32_t x = 1; x <= w; x++) { intg(x, 1).clear(); }
	for (vfc::int32_t y = 0; y <= h; y++) { intg(w, y).clear(); }
	for (vfc::int32_t x = 1; x <= w; x++) { intg(x, h).clear(); }
//#if 1 // multi-threading for feature computation
//	vfc::int32_t l_nrThreads = 4;
//	std::vector<vfc::int32_t> l_startY(l_nrThreads), l_endY(l_nrThreads);
//	vfc::int32_t l_blockSize = (h - 2) / l_nrThreads;
//	//std::vector< std::shared_ptr<std::thread> > m_threads(l_nrThreads);
//	std::vector<std::thread> l_threads(l_nrThreads);
//	for (vfc::int32_t i = 0; i<l_nrThreads; ++i)
//	{
//		l_startY[i] = 1 + i*l_blockSize;
//		//l_endY[i] = h-1 - (l_nrThreads-1-i)*l_blockSize;
//		l_endY[i] = l_startY[i] + l_blockSize;
//		if (l_nrThreads - 1 == i) l_endY[i] = h - 1;
//
//		l_threads[i]
//			= std::thread(&computeGradientMagnitudes,
//				l_startY[i],
//				l_endY[i],
//				w - 1,
//				std::ref(img),
...
//		if (l_nrThreads - 1 == i) l_endY[i] = h + 1;
//
//		l_threads[i] = std::thread(&integrateGradientX,
//			l_startY[i],
//			l_endY[i],
//			1, w + 1,
//			std::ref(intg));
...
//
//
//	std::vector<vfc::int32_t> l_startX(l_nrThreads), l_endX(l_nrThreads);
//	l_blockSize = w / l_nrThreads;
//	for (vfc::int32_t i = 0; i<l_nrThreads; ++i)
//	{
//		l_startX[i] = 1 + i*l_blockSize;
//		//l_endX[i] = w+1 - (l_nrThreads-1-i)*l_blockSize;
//		l_endX[i] = l_startX[i] + l_blockSize;
//		if (l_nrThreads - 1 == i) l_endX[i] = w + 1;
//
//		l_threads[i] = std::thread(&integrateGradientY,
//			l_startX[i],
//			l_endX[i],
//			1, h + 1,
//			std::ref(intg));
...
//#else
	...
	////==============================================
	//      l_startTime = clock();
	for (vfc::int32_t y = 1; y<h - 1; y++)
	{
		//std::cout << "*** Thread number: " << omp_get_thread_num() << std::endl;
		for (vfc::int32_t x = 1; x<w - 1; x++)
		{
			//vfc::int32_t dx = img(x + 1, y) - img(x - 1, y);
			//vfc::int32_t dy = img(x, y + 1) - img(x, y - 1);
			//if (img.type() == CV_8UC1) {
			//	vfc::int32_t dx = vfc::int32_t(img.at<uchar>(y, x + 1) - img.at<uchar>(y, x - 1));
			//	vfc::int32_t dy = vfc::int32_t(img.at<uchar>(y + 1, x) - img.at<uchar>(y - 1, x));
			//	magnitude = ::hypot(dx, dy);
			//	//std::cout << "magnitude: " << ::hypot(dx, dy) << std::endl;
			//}
			//else if (img.type() == CV_32FC1) {
			//	vfc::int32_t dx = vfc::int32_t(img.at<float>(y, x + 1) - img.at<float>(y, x - 1));
			//	vfc::int32_t dy = vfc::int32_t(img.at<float>(y + 1, x) - img.at<float>(y - 1, x));
			//}
			//else {
			//	std::cout << "unknow input image type" << std::endl;
			//	return;
			...
			if (l_interpolateBin)
			{
				...
				intg(x + 1, y + 1).clear();
				intg(x + 1, y + 1).cnt[bin % 9] = vfc::int32_t(lowbin);
				intg(x + 1, y + 1).cnt[(bin + 1) % 9] = vfc::int32_t(highbin);
			}
			else
			{
				vfc::int32_t bin = getBinIdx(dx, dy);
				intg(x + 1, y + 1).clear();
				intg(x + 1, y + 1).cnt[bin % 9] = vfc::int32_t(magnitude);
			}
		}
	}
	...
	for (vfc::int32_t y = 1; y <= h; y++)
		for (vfc::int32_t x = 1; x <= w; x++)
		{
			intg(x, y).add(intg(x - 1, y));
		}

	// cumulate along columns
	//#pragma omp parallel for
	for (vfc::int32_t x = 1; x <= w; x++)
		for (vfc::int32_t y = 1; y <= h; y++)
		{
			intg(x, y).add(intg(x, y - 1));
		}
	//      l_endTime = clock();
//#endif
}

void Fast_HoGFeature::computeCell(vfc::float32_t* vec36,
	vfc::int32_t left, vfc::int32_t top,
	vfc::int32_t w, vfc::int32_t h) {
	const vfc::TImageView<HoGHistogram9>& intg = integralImage_.getView();

	/*left += m_offset.x();
	top += m_offset.y();*/

	for (vfc::int32_t bin = 0; bin<features_dim_; bin++)
	{
		vec36[bin] = vfc::float32_t(+intg(left + w, top + h).cnt[bin] + intg(left, top).cnt[bin]
			- intg(left + w, top).cnt[bin] - intg(left, top + h).cnt[bin]);
	}
}

void Fast_HoGFeature::compute(vfc::float32_t* vec36, bool normalize) {
	if (!variablePartition_)
	{
		// compute all four cells

		/*VFC_REQUIRE(rect.width() % 2 == 0);
		VFC_REQUIRE(rect.height() % 2 == 0);*/

		/*const vfc::int32_t cellW = rect.width() / 2;
		const vfc::int32_t cellH = rect.height() / 2;*/
		const vfc::int32_t cellW = (integralImage_.getWidth() - 3) / 2;
		const vfc::int32_t cellH = (integralImage_.getHeight() - 3)/ 2; 

		computeCell(&vec36[0], 2, 2, cellW, cellH);
		computeCell(&vec36[9], 2 + cellW, 2, cellW, cellH);
		computeCell(&vec36[18], 2, 2 + cellH, cellW, cellH);
		computeCell(&vec36[27], 2 + cellW, 2 + cellH, cellW, cellH);

		// L2-normalize

		const vfc::float32_t epsilon = 0.1f;

		if (normalize)
		{
			vfc::float32_t sum = 0.0;
			for (vfc::int32_t i = 0; i<36; i++)
				sum += vec36[i] * vec36[i];
	...
	{
		vfc::int32_t l_nrFragmentsX = 3;
		vfc::int32_t l_nrFragmentsY = 2;

		vfc::int32_t l_fragmentWidth = std::max((integralImage_.getWidth() - 3)/ l_nrFragmentsX, 1);
		vfc::int32_t l_fragmentHieght = std::max((integralImage_.getHeight() - 3)/ l_nrFragmentsY, 1);
		//std::cout<<"rect width "<<rect.width()<<" frag width "<<l_fragmentWidth<<std::endl;
		vfc::int32_t l_fragmentStepX = l_fragmentWidth;
		vfc::int32_t l_fragmentStepY = l_fragmentHieght;
		vfc::int32_t l_nrFragments = 0;
		for (vfc::int32_t x = 0; x<l_nrFragmentsX; x++)
		{
			for (vfc::int32_t y = 0; y<l_nrFragmentsY; y++)
			{
				computeCell(&vec36[l_nrFragments * 9], 1 + x*l_fragmentStepX, 1 + y*l_fragmentStepY,
					l_fragmentWidth, l_fragmentHieght);
				++l_nrFragments;
			}
		}
		...
		l_fragmentWidth = std::max((integralImage_.getWidth() - 3) / l_nrFragmentsX, 1);
		l_fragmentHieght = std::max((integralImage_.getHeight() - 3) / l_nrFragmentsY, 1);
		l_fragmentStepX = l_fragmentWidth;
		l_fragmentStepY = l_fragmentHieght;
		for (vfc::int32_t x = 0; x<l_nrFragmentsX; x++)
		{
			for (vfc::int32_t y = 0; y<l_nrFragmentsY; y++)
			{
		...
		//l_fragmentWidth = std::max(rect.width() / l_nrFragmentsX - 1, 1);
		//l_fragmentHieght = std::max(rect.height() / l_nrFragmentsY - 1, 1);
		//l_fragmentStepX = l_fragmentWidth;
		//l_fragmentStepY = l_fragmentHieght;
		//for (vfc::int32_t x = 0; x<l_nrFragmentsX; x++)
		//{
		//    for (vfc::int32_t y = 0; y<l_nrFragmentsY; y++)
		//    {
		//        computeCell(&vec36[l_nrFragments * 9], rect.left() + x*l_fragmentStepX, rect.top() + y*l_fragmentStepY,
		//            l_fragmentWidth, l_fragmentHieght);
		//        ++l_nrFragments;
		//    }
		//}

		//// add whole pach as one fragment
		//computeCell(&vec36[l_nrFragments * 9], rect.left(), rect.top(),
		//            rect.width(), rect.height());
		//++l_nrFragments;

		//vfc::int32_t l_nrFeatures = l_nrFragments*9;
		//std::cout<<"l_nrfragments "<<l_nrFragments<<" #features "<<l_nrFeatures<<std::endl;
		const vfc::float32_t epsilon = 0.1f;
		...
		if (normalize)
		{
			...
			// local normalization
			for (vfc::int32_t i = 0; i<l_nrFragments; ++i)
			{
				vfc::float32_t sum = 0.0;
				for (vfc::int32_t j = 0; j<9; ++j)
				{
					// L1
					sum += vec36[j + i * 9];
					//// L2
					//sum += vec36[j+i*9]*vec36[j+i*9];
				}
				////L2
				//sum = ::sqrt(sum + epsilon);
				sum += epsilon;
...
void Fast_HoGFeature::compute(vfc::float32_t* vec36, vfc::int32_t cell_size, bool normalize){
	

	const vfc::TImageView<HoGHistogram9>& intg = integralImage_.getView();
	vfc::int32_t featureCounter = 0;
	for (vfc::int32_t i = 0; i < sizeY_; i++) {
		for (vfc::int32_t j = 0; j < sizeX_; j++) {
			computeCell(&vec36[featureCounter], j * cell_size_ + 1 + cell_size_, i * cell_size + 1 + cell_size_, cell_size_, cell_size_);
			featureCounter += features_dim_;
		}
	}
	const vfc::float32_t epsilon = 0.1f;
	
	if (normalize) {
		for (vfc::int32_t i = 0; i < sizeX_ * sizeY_; i++) {
			vfc::float32_t sum = 0;
			for (vfc::int32_t j = 0; j < features_dim_; j++) {
				sum += vec36[j + i * features_dim_];
			}
			// L1 normalizetion
			sum += epsilon;
			for (vfc::int32_t j = 0; j < features_dim_; ++j)
			{
				vfc::float32_t(vec36[j + i * features_dim_]) /= vfc::float32_t(sum);
			}
		}
	}
}

std::vector<cv::Mat> Fast_HoGFeature::computeHOGFeature(const std::vector<cv::Mat> ims) {
	if (ims.empty())
	{
	...
	for (vfc::int32_t i = 0; i < ims.size(); i++) {
		if (ims[i].empty())
		{
			std::cout << "image is empty !!!" << std::endl;
		}

		cv::Mat	img_input_temp;
		ims[i].copyTo(img_input_temp); // ims[i]: CV_8UC3
		
		if (img_input_temp.channels() == 3) {
			cv::cvtColor(img_input_temp, img_input_temp, cv::COLOR_BGR2GRAY); // img_input_temp: CV_8UC1
		}	

		bool cellSize_fixed = true;
		if (cellSize_fixed) {
			cv::Size _tmpl_sz;
			_tmpl_sz.width = img_input_temp.cols;
			_tmpl_sz.height = img_input_temp.rows;

			// Round to cell size and also make it even
			if (int(_tmpl_sz.width / (cell_size_)) % 2 == 0)
			{
				_tmpl_sz.width = ((int)(_tmpl_sz.width / (2 * cell_size_)) * 2 * cell_size_) + cell_size_ * 2;
				_tmpl_sz.height = ((int)(_tmpl_sz.height / (2 * cell_size_)) * 2 * cell_size_) + cell_size_ * 2;
			}
			else
			{
				_tmpl_sz.width = ((int)(_tmpl_sz.width / (2 * cell_size_)) * 2 * cell_size_) + cell_size_ * 3;
				_tmpl_sz.height = ((int)(_tmpl_sz.height / (2 * cell_size_)) * 2 * cell_size_) + cell_size_ * 3;
			}

			if (img_input_temp.cols != _tmpl_sz.width || img_input_temp.rows != _tmpl_sz.height)
			{
				img_input_temp.resize(_tmpl_sz.width, _tmpl_sz.height);
			}
			
			sizeX_ = _tmpl_sz.width / cell_size_ - 2;
			sizeY_ = _tmpl_sz.height / cell_size_ - 2;			
			
		}
		else {
			if (!variablePartition_) {
				img_input_temp.resize(img_input_temp.cols / 2 * 2, img_input_temp.rows / 2 * 2);
			}
		}

		vfc::float32_t* vec36 = (vfc::float32_t *)malloc(sizeof(vfc::float32_t) * (sizeX_ * sizeY_ * features_dim_));
		computeIntegralHistogram(img_input_temp, integralImage_);
		//compute(vec36, normalized_);
		compute(vec36, cell_size_, normalized_);

		// derive the CvMat directly from the 1dim feature vector, easier!
		std::vector<cv::Mat> result_maps;
		cv::Mat feature_map = cv::Mat(sizeY_, sizeX_, CV_32FC(features_dim_), vec36);
		cv::split(feature_map, result_maps);

		for (vfc::int32_t j = 0; j < result_maps.size(); j++) {
			hog_feats.push_back(result_maps[j]);
		}
		
		// pixelweise value assignments
		for (vfc::int32_t f = 0; f < features_dim_; f++) {
			cv::Mat temp_s(sizeY_, sizeX_, CV_32FC1);
			for (vfc::int32_t m = 0; m < sizeY_; m++) {
				for (vfc::int32_t n = 0; n < sizeX_; n++) {
					temp_s.at<float>(m, n) = vec36[m * sizeY_ * features_dim_ + n * features_dim_ + f];
				}
			}
			//hog_feats.push_back(temp_s);
		}	
